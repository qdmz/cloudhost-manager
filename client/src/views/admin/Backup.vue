    ...
    <a-row :gutter="16">
      <a-col :span="12">
        <a-card title="数据库备份">
          <a-space style="margin-bottom:16px">
            <a-button type="primary" @click="handleCreateBackup" :loading="creating">立即备份</a-button>
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'action'">
                ...
                <a-popconfirm title="确定要恢复此备份吗？这会覆盖当前数据库！" @confirm="handleRestore(record)" style="margin-left:8px">
                  <a-button size="small" danger>恢复</a-button>
                </a-popconfirm>
                <a-popconfirm title="确定要删除此备份吗？" @confirm="handleDelete(record)" style="margin-left:8px">
      ...
      <a-col :span="12">
        <a-card title="一键操作">
          ...
          <a-button type="primary" danger @click="handleFullRestore" :loading="restoring" style="margin-top:16px">
            一键恢复（最新备份）
          </a-button>
        </a-card>
      </a-col>
...
import { getBackups, createBackup, deleteBackup, restoreBackup as apiRestoreBackup, downloadBackup } from '@/api/admin'
...
const handleCreateBackup = async () => {
  creating.value = true
  try {
    await createBackup({ type: backupType.value })
