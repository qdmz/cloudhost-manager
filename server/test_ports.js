...
async function test() {
  var svc = await Service.findOne({ where: { id: 1 } });
  console.log("Service 1 user_id:", svc.user_id);
  
  var token3 = jwt.sign({ userId: 3 }, process.env.JWT_SECRET, { expiresIn: "1h" });
  try {
    var r = await axios.get("http://localhost:8111/api/services/5", {
